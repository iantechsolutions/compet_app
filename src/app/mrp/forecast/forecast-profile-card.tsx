...
export default function ForecastProfileCard(props: {
    ...
    const clientsWithBudgets = useMemo(() => {
        return crmClients.filter(client => quantityByClient.has(client.client_id) && !profile.clientInclusionList?.includes(client.client_id.toString()))
    ...
    return <li>
        <Card>
            ...
            <CardContent>
                ...
                {(profile.clientInclusionList && clientsWithBudgets) && <p className="text-sm font-medium">Clientes:
                    <ListSelectionDialog
                        readOnly
                    ...
                    >
                        <a href="javascript:void(0)" className="ml-1 text-blue-500 underline">({clientsWithBudgets.length} seleccionados)</a>
                    </ListSelectionDialog>
                </p>}
            ...
            <CardFooter>
                {!profile.current && <Button
                    ...
                    onClick={() => props.handleApplyProfile(profile.id)}
                >Aplicar</Button>}
                {profile.current && <Button
                    disabled
