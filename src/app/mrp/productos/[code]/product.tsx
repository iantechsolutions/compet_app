                            ...
                            <TableCell colSpan={6} className='font-medium pt-7'>
                                <div className='flex items-center'>
                                    ...
                                    <span className='ml-3 text-xs'>
                                        Stock inicial: {formatStock(product.stock)} - Stock comprometido: {formatStock(product.commited)} =
                                        Stock disponible: {formatStock(product.stock - product.commited)}
                                    </span>
                                </div>
                            </TableCell>
                        </TableRow>
                            ...
                            return (
                                <Fragment key={i}>
                                    <TableRow key={`header:${month}`}>
                                        <TableCell colSpan={6} className='font-medium pt-7'>
                                            <div className='flex items-center'>
                                                ...
                                                <span className='ml-3 text-xs'>
                                                    Stock inicial: {formatStock(initialStock)}, Variación stock:{' '}
                                                    {formatStock(stockVariation)}, stock final: {formatStock(finalStock)}
                                                </span>
                                            </div>
                                        </TableCell>
                                    </TableRow>
                                    {forecastEvents.map((event, i) => {
                                        return (
                                            <ProductEventRow key={`row:${month}:f_${i}`} event={event} productCode={productCode} nostock />
                                        )
                                    })}
                                    <ForecastSupplyEventsRow
                                        events={p.supplyForecastEvents}
                                        month={month}
                                        key={`forecast_supply_event_row:${month}`}
                                    />
